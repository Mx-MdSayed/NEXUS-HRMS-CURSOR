  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-surface-500 dark:text-surface-400">
            Centralized approvals for HRMS requests.
          </p>
        </div>
        <Link to="/workflows/requests">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardContent>
            <p className="text-sm text-surface-500">Needs information</p>
            <p className="font-display text-3xl font-semibold">{needsInfo}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent>
            <p className="text-sm text-surface-500">Completed / approved</p>
            ...
            <TableBody>
              {rows.slice(0, 8).map((row) => (
                <TableRow key={row.id}>
                  ...
                  <TableCell>
                    <Link to={`/workflows/requests/${row.id}`}>
